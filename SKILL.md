---
name: "excel-win32com"
description: "使用 win32com 操作 Excel 文件，支持删除指定行及其关联图片、交换任意两行（含图片）、多 Sheet 批量操作、统一单元格格式、提取行到新 Sheet。"
---
...
### `scripts/normalize_desc_format.py`
统一指定列的内容格式，将 `*` 符号替换为数字序号，同时统一字体、自动换行、列宽、行高。

### `scripts/extract_rows_to_sheet3.py`
将多个 Sheet 的指定行提取到新 Sheet（Sheet3），含图片复制、列宽设置、自动换行、图片尺寸自适应。
